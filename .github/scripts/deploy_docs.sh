...
if [ "$GUIDE_SSH_KEY" != "" ]; then
    ...
    if [ "$release_snapshot" != "true" ]; then
        rsync -avz target/apidocs guides@deepsymmetry.org:/var/www/html/beat-carabiner-java
    else
        rsync -avz target/apidocs guides@deepsymmetry.org:/var/www/html/beat-carabiner-java/snapshot
    fi
else
